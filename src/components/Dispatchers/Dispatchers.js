...
function Dispatchers() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const token = localStorage.getItem("access");
        const response = await fetch(`${_apiBase}/accounts/filter-by/?group=Dispatcher`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `JWT ${token}`,
  ...
  const renderHeader = () => {
    return (
      <div className="d-flex flex-wrap gap-2 justify-content-between align-items-center">
        <h5 className="m-0">Диспетчера</h5>
    ...
    <div className="container container__couriers card mt-4">
      ...
      <Spin className="spin" spinning={loading} size="large">
        <DataTable
          ...
          currentPageReportTemplate="Показано с {first} по {last} диспетчер из {totalRecords}"
        >
...
export default withLayout(Dispatchers);
