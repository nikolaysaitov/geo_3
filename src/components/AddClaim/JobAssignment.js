...
function JobAssignment() {
  ...
  const JobAssignmentSchema = yup.object().shape({
  ...
  const showInfo = () => {
    toast.current.show({ severity: "success", summary: "Спасибо", detail: "Задание на перевозку отправлено", life: 7000 });
  };
  const showError = () => {
    toast.current.show({
      severity: "error",
  ...
  const {
    ...
    resolver: yupResolver(JobAssignmentSchema),
  });

  async function onSubmitJobAssignment(data1) {
    try {
      setLoading(true);
...
const onChange = (e) => {
  ...
  return (
    <form className="container  mt-5" onSubmit={handleSubmit(onSubmitJobAssignment)}>
      <Spin className="spin" spinning={loading} size="large">
        ...
        <Divider />
        <div className="form_block">
          <div className="input_add_claim_block  col-sm-5">
          ...
          <div className="input_add_claim_block col-sm-7">
