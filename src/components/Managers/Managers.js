...
function Managers() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const token = localStorage.getItem("access");
        const response = await fetch(`${_apiBase}/accounts/filter-by/?group=Manager`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `JWT ${token}`,
    ...
    <div className="container container__couriers card mt-4">
      ...
      <Spin className="spin" spinning={loading} size="large">
        <DataTable
          ...
          currentPageReportTemplate="Показано с {first} по {last} менеджер из {totalRecords}"
        >
...
export default withLayout(Managers);
